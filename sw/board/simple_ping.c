...
void	user_task(void) {
	...
	while(1) {
		do {
			...
			while(((_net1->n_rxcmd & ENET_RXAVAIL)==0)
					&&(_rtc->r_clock == rtc))
				user_heartbeats++;
		} while((_net1->n_rxcmd & ENET_RXAVAIL)==0);
		...
		int	invalid = 0;
		int	ln, rxcmd = _net1->n_rxcmd;

		ln = _net1->n_rxcmd & 0x07ff;
		for(int k=0; k<(ln+3)>>2; k++)
			rxpkt[k] = _netbrx[k];
		epayload = &rxpkt[2];
		_net1->n_rxcmd = ENET_RXCLR|ENET_RXCLRERR;
		*_spio = LED_RXCLEAR;
...
void	send_ping(void) {
	...
	*_spio = LED_MAC;
	...
	if (_net1->n_txcmd & ENET_TXBUSY) {
		*_spio = LED_BUSY;
		while(_net1->n_txcmd & ENET_TXBUSY)
			;
		*_spio = LED_NOTBUSY;
	}
...
int main(int argc, char **argv) {
	...
	*_spio = LED_STARTUP;
	...
	if ((_net1->n_txcmd & ENET_RESET)!=0)
		_net1->n_txcmd = 0; // Turn on all our features
	{
		// volatile unsigned long *emac
		//	= (volatile unsigned long *)&_net1->n_mac;
		// *emac = my_mac_addr;
		_net1->n_mac = my_mac_addr;
	}
