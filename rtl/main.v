...
// check
	...
	// from
`ifdef	FLASH_ACCESS
	localparam	RESET_ADDRESS = 4259840;
`else
	localparam	RESET_ADDRESS = 65536;
`endif
	//
...
// `include "builddate.v"
	...
	assign	   buildtime_sel = ((wb_sio_sel)&&(wb_addr[ 3: 0] ==  4'h0));  // 0x0000
	assign	      buserr_sel = ((wb_sio_sel)&&(wb_addr[ 3: 0] ==  4'h1));  // 0x0004
	assign	      buspic_sel = ((wb_sio_sel)&&(wb_addr[ 3: 0] ==  4'h2));  // 0x0008
	assign	        gpio_sel = ((wb_sio_sel)&&(wb_addr[ 3: 0] ==  4'h3));  // 0x000c
	assign	    netrxctr_sel = ((wb_sio_sel)&&(wb_addr[ 3: 0] ==  4'h4));  // 0x0010
	assign	    nettxctr_sel = ((wb_sio_sel)&&(wb_addr[ 3: 0] ==  4'h5));  // 0x0014
	assign	    pwrcount_sel = ((wb_sio_sel)&&(wb_addr[ 3: 0] ==  4'h6));  // 0x0018
	assign	        spio_sel = ((wb_sio_sel)&&(wb_addr[ 3: 0] ==  4'h7));  // 0x001c
	assign	     version_sel = ((wb_sio_sel)&&(wb_addr[ 3: 0] ==  4'h8));  // 0x0020
	assign	        uart_sel = ((wb_addr[14:10] &  5'h1f) ==  5'h01); // 0x1000 - 0x100f
	assign	        net1_sel = ((wb_addr[14:10] &  5'h1f) ==  5'h02); // 0x2000 - 0x201f
	assign	         rtc_sel = ((wb_addr[14:10] &  5'h1f) ==  5'h03); // 0x3000 - 0x301f
	assign	      wb_sio_sel = ((wb_addr[14:10] &  5'h1f) ==  5'h04); // 0x4000 - 0x403f
//x2	Was a master bus as well
	assign	       mdio1_sel = ((wb_addr[14:10] &  5'h1f) ==  5'h05); // 0x5000 - 0x507f
	assign	        netb_sel = ((wb_addr[14:10] &  5'h18) ==  5'h08); // 0x8000 - 0xffff
	assign	       bkram_sel = ((wb_addr[14:10] &  5'h10) ==  5'h10); // 0x10000 - 0x1ffff
	//
	...
	assign	     wbu_dwb_sel = ((wbu_addr[15:15] &  1'h1) ==  1'h0); // 0x0000 - 0x1ffff
//x2	Was a master bus as well
	assign	     zip_dbg_sel = ((wbu_addr[15:15] &  1'h1) ==  1'h1); // 0x20000 - 0x20007
	//
	...
	if (wb_stb && ! wb_stall)
		casez(wb_addr[14:10])
			// 0001f000 & 00001000, uart
			5'b0_0001: r_wb_bus_select <= 3'd0;
			// 0001f000 & 00002000, net1
			5'b0_0010: r_wb_bus_select <= 3'd1;
			// 0001f000 & 00003000, rtc
			5'b0_0011: r_wb_bus_select <= 3'd2;
			// 0001f000 & 00004000, wb_sio
			5'b0_0100: r_wb_bus_select <= 3'd3;
			// 0001f000 & 00005000, mdio1
			5'b0_0101: r_wb_bus_select <= 3'd4;
			// 00018000 & 00008000, netb
			5'b0_1???: r_wb_bus_select <= 3'd5;
			// 00010000 & 00010000, bkram
			5'b1_????: r_wb_bus_select <= 3'd6;
			default: begin end
		endcase

	always @(posedge i_clk)
	casez(r_wb_bus_select)
		3'd0: wb_idata <= uart_data;
		3'd1: wb_idata <= net1_data;
		3'd2: wb_idata <= rtc_data;
		3'd3: wb_idata <= wb_sio_data;
		3'd4: wb_idata <= mdio1_data;
		3'd5: wb_idata <= netb_data;
		3'd6: wb_idata <= bkram_data;
		default: wb_idata <= bkram_data;
	endcase

	assign	wb_stall =	((uart_sel)&&(uart_stall))
				||((net1_sel)&&(net1_stall))
				||((rtc_sel)&&(rtc_stall))
...
`ifdef	NET1_ACCESS
	enetpackets	#(.MEMORY_ADDRESS_WIDTH(14))
		net1i(i_clk, i_reset,
			...
			o_net1_reset_n,
			i_net1_rx_clk, i_net1_rx_dv, i_net1_rx_err, i_net1_rxd,
			i_clk_125mhz, o_net1_tx_clk, o_net1_tx_ctl, o_net1_txd,
...
`endif
	...
	wbuconsole genbus(i_clk, wbu_rx_stb, wbu_rx_data,
			...
			wbubus_dbg[0]);
	assign	wbu_sel = 4'hf;
	assign	wbu_addr = wbu_tmp_addr[(16-1):0];
`else	// WBUBUS_MASTER
...
`endif	// WBUBUS_MASTER
	...
	assign	buserr_data = { {(32-2-15){1'b0}},
			r_buserr_addr, 2'b00 };
`ifdef	INCLUDE_ZIPCPU
	//
	//
...
`ifdef	INCLUDE_ZIPCPU
	...
	wbpriarbiter #(32,15)	bus_arbiter(i_clk,
...
`ifdef	BUS_DELAY_NEEDED
	busdelay #(15)	wbu_dwbi_delay(i_clk, i_reset,
		wbu_dwbi_cyc, wbu_dwbi_stb, wbu_dwbi_we, wbu_dwbi_addr, wbu_dwbi_odata, wbu_dwbi_sel,
