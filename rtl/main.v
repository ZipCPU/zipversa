...
// `include "builddate.v"
	...
	assign	   buildtime_sel = ((wb_sio_sel)&&(wb_addr[ 2: 0] ==  3'h0));  // 0x0000
	assign	      buserr_sel = ((wb_sio_sel)&&(wb_addr[ 2: 0] ==  3'h1));  // 0x0004
	assign	      buspic_sel = ((wb_sio_sel)&&(wb_addr[ 2: 0] ==  3'h2));  // 0x0008
	assign	    pwrcount_sel = ((wb_sio_sel)&&(wb_addr[ 2: 0] ==  3'h3));  // 0x000c
	assign	        spio_sel = ((wb_sio_sel)&&(wb_addr[ 2: 0] ==  3'h4));  // 0x0010
	assign	     version_sel = ((wb_sio_sel)&&(wb_addr[ 2: 0] ==  3'h5));  // 0x0014
	assign	        uart_sel = ((wb_addr[14:12] &  3'h7) ==  3'h1); // 0x4000 - 0x400f
	assign	         rtc_sel = ((wb_addr[14:12] &  3'h7) ==  3'h2); // 0x8000 - 0x801f
	assign	      wb_sio_sel = ((wb_addr[14:12] &  3'h7) ==  3'h3); // 0xc000 - 0xc01f
//x2	Was a master bus as well
	assign	       bkram_sel = ((wb_addr[14:12] &  3'h4) ==  3'h4); // 0x10000 - 0x1ffff
	//
...
//x2	Was a master bus as well
	...
	// connected to them.
	//
	reg [1:0]	r_wb_bus_select;
	always	@(posedge i_clk)
	if (wb_stb && ! wb_stall)
		casez(wb_addr[14:12])
			// 0001c000 & 00004000, uart
			3'b0_01: r_wb_bus_select <= 2'd0;
			// 0001c000 & 00008000, rtc
			3'b0_10: r_wb_bus_select <= 2'd1;
			// 0001c000 & 0000c000, wb_sio
			3'b0_11: r_wb_bus_select <= 2'd2;
			// 00010000 & 00010000, bkram
			3'b1_??: r_wb_bus_select <= 2'd3;
			default: begin end
		endcase

	always @(posedge i_clk)
	casez(r_wb_bus_select)
		2'd0: wb_idata <= uart_data;
		2'd1: wb_idata <= rtc_data;
		2'd2: wb_idata <= wb_sio_data;
		2'd3: wb_idata <= bkram_data;
